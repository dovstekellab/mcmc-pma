...
mcmc_simplebaranyi2::mcmc_simplebaranyi2(std::vector<double> od, std::string l) : mcmc(od, l) 
...

{
    ...
    double startv[] = {odlag(),ody0(),odymax(),1,1};
    double p[] = {0.5,0.1,0.1,0.0001,0.00005};
...
bool mcmc_simplebaranyi2::do_before_run(){
    
    startparameters[3] = odr(startparameters, 3, 0.01, 1);
    startparameters[4] = odoptimiseparameter(startparameters, 4, 1, 10);
    if (startparameters[4]<0) startparameters[4]=0;
    return TRUE;
}

double mcmc_simplebaranyi2::results_lagbyparams(std::vector<double> parameters){
    double t = 0;
...
std::vector<double> mcmc_simplebaranyi2::md(std::vector<double> parameters){
    ...
    for (int i=0; i < observed_data.size(); i++) {
        ...
        model_data[i] = exp(
                            ...
                            p[3] * i
                            +
                            log( (exp(-p[3]*i) + q0)/(1+q0) )
                            -
                            1/p[4] * log( 1 + 
                                ( exp(p[4] *  p[3]*i + p[4] * log((exp(-p[3]*i) + q0)/(1+q0)) ) - 1 ) 
                                / 
...
bool mcmc_simplebaranyi2::checkparameters(std::vector<double> parameters){
    std::vector<double> p = logtolin(parameters);
